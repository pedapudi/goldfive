...
| 39 | `LLM_CALL_TIMEOUT` | `goldfive/adapters/_adk_plugin.py` per-call watcher (`make_adk_plugin(llm_call_timeout_ms=...)`, default 120s). #476: under `observation_only` it no longer cancels the invocation and a one-shot reasoning-channel-disarm warning is emitted. | CRITICAL | default |
