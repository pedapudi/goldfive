...
| `refine_outcomes` | `dict[tuple[str,str], RefineOutcome]` | `DefaultSteerer._handle_drift` / `_apply_revision` | intervention ladder, REPEATED_FAILURE | **`DriftObserver.reset_for_turn`** (per turn, after `run_started`) |
...
3. **`Runner.run` fires `run_started`**, then calls `DriftObserver.reset_for_turn(session)` → `session.refine_outcomes.clear()`. This is the reset point for the per-turn retry budget.
...
4. If it needs an explicit per-turn reset (not just fresh-Session), wire the clear into `DriftObserver.reset_for_turn` (the `refine_outcomes` model) and document that as the reset point.
...
| Retry budget carries stale failures into a new turn | `refine_outcomes` not reset | confirm `DriftObserver.reset_for_turn` runs after `run_started` |
